...
subtile: "Session 3 practical exercises"
---
...
Do not panic. This is R being honest with you.
When R tries to convert a character column to numeric, it succeeds for values like `"2"` or `"8"`. But some cells may contain text that cannot be converted — like a genuine missing value entered as blank or a data entry error. R converts those to `NA` and tells you about it.
...
- Many otrher `as.something()` functions exist that cover topics beyond the course (matrix, tibbles, vectors, etc.)
