...
Your head of department, Kassandra, has just handed you a small sample of the unit's surveillance data set (`imd_S1.xlsx`) — a few dozen cases from the invasive meningococcal disease registry. They are still working on your data permit, but this linelist can be useful for you to become acquainted with the data and get some practice before the actual work begins in a few days.
...
Make sure `imd_raw` is loaded in your Environment before you start. If it is not, go back to part 6 and re-run your import lines.
...
Vectors are a sequence of values living together in a single object. R is a vectorised programming language, meaning that information is processed through these. In fact, your entire dataframe is built as a collection of columns representing variables, with each column constituting a vector itself.
...
This means your dataframe `imd_raw` is a single object that contains multiple vectors. To extract a single column from a dataframe, use the `$` operator like this:
...
And, since we have the object in our Environment, let's look at something else. Before, you used `nrow()` to ask how many observations/rows the data had. Now we have a vector, and we want to know how many values it contains, for which we have a function called `length()`.
...
This is not a minor question. Creating an object allows you to keep a result and use it later. However, is it always necessary to keep the result? Sometimes you may just want to quickly check something, but the information serves no other purpose. So always think about the key distinction between printing a result and assigning it to an object in R.
:::
...
**Action** — What is the mean `age` of your current cases? Use your age object and compare the result to the data column. 
...
**You are done with the Session 1 practicals.** You now have a working R Project, your packages loaded, your data imported, and your first functions under your belt. In Session 2, you will meet Kassandra and learn about the true power of R: the Tidyverse.
...
| **`mean()` , `sum()` , `min()` , `max()`** | Calculate numerical summaries of a vector |
