...
Let's use the functions from the `dplyr` package to fix two variables: `sex` and `type_of_case`. With dplyr you can either recode or replace values. Usually we `recode_values()` when we're creating an entirely new variable. And we use `replace_values()` when partially updating an existing one. Most important distincion is what happens with non-matched values. In recode, we define that in the end, in replace they stay the same as they were. Both functions map old values to new ones using a clean formula syntax: <!--# recode is from older version of dplyr... now type_of_case needs replace_values(). recode_values as used in solution will only recode Sec to Second, everything else will be miss...  -->
...
replace_values(column, 
               old_value         ~ "New Value",  # for single values
               c("old1", "old2") ~ "New Value")  # for multiple values at once
                                                 # non-matched values stay the same
```
...
| `recode_values(), replace_values()` | dplyr | Maps old values to new ones using formula syntax |
