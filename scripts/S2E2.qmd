...
Finding missing values is a common task when doing data cleaning, but that is tomorrow's topic. For now, we are also interested in discarding cases with `NA` values in some of its variables, something you will also do quite frequently in the future - believe me. For that, we can introduce the last of today's operators, the `!` - opposite operator
