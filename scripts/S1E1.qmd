...
**Action** — Now it's time to create your first object `disease` which contains the character "salmonella". Try out and then call the object to see the difference
...
Before, you created an object named `disease` that contained the word "salmonella". Maybe you did not realize then, but something changed in the upper-right window: the **environment**. There, all the information you work with during your session is stored.
...
**Action** — Create a new object named `result` and assign to it the addition `operation + value`. The go back to that code line, and execute it again. And again. Once more. Does it change the output?
...
| **Execute code** | Code run from the scrip using "run" button or Ctrl /Cmd + Enter |
| **Print code** | Code that is executed without assignment that is shown on the console and then dissapears |
