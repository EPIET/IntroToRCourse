...
imd <- import(here("data", "IMD_Sample_Clean.csv"))
```
...
imd_filtered_f_adult <- filter(imd_filtered_f, age >= 18)
```

And again, let's break it down

- As a sequence, every step relied in the previous result: first we filtered the region, then that new dataframe was filtered for sex, and the resulting data for age. Every new step required the creation of a new object, and using it in the next function
- Each filtering used the adequate variable: `region_id`, `sex` and `age` and a logical operator
...
``` r
filter(filter(filter(imd, region == "R6"), sex == "Female"), age >= 18)
```

Now when you read this, your head needs a few minits to understand what is happening. To really come up with the sequence, you need to start reading in the middle where `imd` is called for the first time, then it becomes a serires of nested functions, arranged inside-out. Not beautiful, but does the work in a single command, and saves you three objects
...
> Take `imd`, **and then** keep only female cases, **and then** keep only cases from R6, **and then** count the rows.
