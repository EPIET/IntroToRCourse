...
- If two variables have slightly different names, i.e., `sex` and `Sex`, the function will fill in the empty spots with NAs

- If variable class is different, the function will try to coerce one of the two to the other, with unknown results

Always double check the result of `bind_rows()` before assuming it worked because you did not get any error or warning
:::

<!--# not really true, i.e. if we bind them, sex for total will just have NA, with no warning or error JAVI: you are right, those were the condictions for base rbind(); updated -->
