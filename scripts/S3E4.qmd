...
imd_cleaning <- imd_raw %>%
  ...
  mutate(
    ...
    case_classification = case_when(
      ...
      lab_confirmed == "No" & !is.na(type_of_case) & 
        !is.na(clinical_presentation)  ~ "Probable",
      
      lab_confirmed == "No" & is.na(type_of_case) & 
        !is.na(clinical_presentation)  ~ "Suspected",
      
      TRUE ~ "Discarded"
    )
  ) %>%
  # final filter of excluded cases
  filter(
    disease == "Mening",
    imported == "No",
