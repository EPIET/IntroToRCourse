...
**Action** — Create the variable `mean_age` using `mutate()` and the `mean()` function. Assign it to the same `imd` dataframe you are starting from.
...
Since we defined `adult` as a numeric 0-1 variable, the `sum()` function was pretty straightforward, as it added up the `1` and `0` values resulting in the total number of 1s.
...
imd_adults <- imd %>% 
  mutate(
    ...
    total_female = sum(sex == "Female", na.rm = T), # you need na.rm bc some values are miss
